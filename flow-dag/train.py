...
def train_lower(epoch, best_val_loss, ground_truth_G, lambda_A, c_A, optimizer):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        # log_alpha = sinkhorn_net(data)
        # soft_perms_inf, log_alpha_w_noise = gumbel_sinkhorn(log_alpha, temp=args.temp)
        # inv_soft_perms = inv_soft_pers_flattened(soft_perms_inf)
        # x_permute = torch.matmul(soft_perms_inf, data)
    ...
    return np.mean(np.mean(kl_train)  + np.mean(nll_train)), np.mean(nll_train), np.mean(mse_train), graph, origin_A, Adj, permute
