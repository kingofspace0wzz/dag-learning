...
def simple_sinkhorn(A, n_iter=20):
...
def inv_soft_pers_flattened(soft_perms_inf, n_numbers=10):
  # inv_soft_perms = torch.transpose(soft_perms_inf, 2, 3)
  # inv_soft_perms = torch.transpose(inv_soft_perms, 0, 1)
  inv_soft_perms_flat = torch.transpose(soft_perms_inf, -2, -1)
...
if __name__ == "__main__":
  ...
  # keep_prob = 1.
  # n_epochs = 500
  # model = sorting_model.Sinkhorn_Net(latent_dim= n_units, output_dim= n_numbers, dropout_prob = 0.)
  # criterion = nn.MSELoss()
  # optimizer = torch.optim.Adam(model.parameters(), lr=0.001, eps=1e-8)

  # train_set = my_sample_uniform_and_order(2, 5, 1.)
  # [train_ordered, train_random, train_hard_perms] = [train_set[i] for i in range(3)]
  ...
  # soft_perms_inf, log_alpha_w_noise = gumbel_sinkhorn(log_alpha, temperature, 1, noise_factor,  n_iter_sinkhorn, squeeze=True)
